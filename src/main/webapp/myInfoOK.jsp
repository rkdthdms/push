...
//	Servers 폴더의 server.xml 파일을 열고 Connector 태그에 URLEncoding="UTF-8" 속성을 추가하면 
...
//	이전 페이지에서 넘어오는 데이터는 무조건 문자열 형태로 넘어온다. 
	...
	try {
		...
		out.println(age + "님은 올해 " + age + "살 입니다.<br>");
		out.println(age + "님은 내년에 " + (age + 1) + "살 입니다.<br>");
...
//	checkbox에서 넘어오는 데이터를 받을 때 넘어오는 항목이 여러 개일 수 있기 때문에 request.getParameterValues()
//	메소드로 받아서 
	...
	out.println(name + "님의 취미는 " + travel + "입니다.<br>");
...
//	 태그 사용 가능, 줄바꿈 불가능
	out.println(content);
//	out.println(content.replace("<", "&lt;").replace(">", "&gt;") + "<br>").replace("\r\n", "<br>" + "<br>");
